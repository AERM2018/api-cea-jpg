...
const getTestInfo = async(forMakingActa = false, opts = {id_group : 0,id_course:0}) => {
                ...
                include : [
                    {
                        ...
                        attributes : ['id_group','name_group'],
                            include:{
                                model:Major,
                                attributes:['id_major',[fn('concat',col('gro_cou->groupss->major->educational_level.educational_level'),' en ',col('gro_cou->groupss->major.major_name')),'major_name']],
                                include:{
    ...
    if(forMakingActa){
        ...
        tests = {...restGroup,major_name:tests[0].gro_cou.groupss.major.major_name,...tests[0].gro_cou.course,...teacher,tests:tests.map(({major,gro_cou,student,...test}) => ({...test,...student}))}
    }else{
        tests = tests.map(({gro_cou,student,...rest}) => {
            const {groupss:{major,...restGroup},course} = gro_cou
            return {...rest,...student,...restGroup,major_name:major.major_name,...course}
        })
