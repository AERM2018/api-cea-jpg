...
const isValidSchedule = (req, res = response, next) => {
  ...
  const invalidDay = time_table.find(
    ({ day }) => day < 0 || day > 6 || day === null
  );
  const invalidHour = time_table.find(({ start_hour, finish_hour }) => {
    const start = start_hour.split(":");
    const finish = finish_hour.split(":");

    if (
      start.map((time) => Number(time)).includes(NaN) ||
      finish.map((time) => Number(time)).includes(NaN)
    ) {
      return true;
    }
