...
class ServiceLetter extends AlePDFDocument{
    ...
    fillDocument(){
        ...
        .font('regular')
        .text(`en esa institución a su digno cargo a ${(this.student.gendre == 'F')?'la':'el'} C.`,{continued:true})
        .font('regular-bold')
        .text(`${this.student.student_name} `,{continued:true})
        .font('regular')
        .text(`es alumn${(this.student.gendre == 'F')?'a':'o'} de este instituto que ha sido aprobad${(this.student.gendre == 'F')?'a':'o'} para hacer ${(this.type == 'practicas')?'sus':'su'} `,{continued:true})
        .font('regular-bold')
